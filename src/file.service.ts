...
export class FileService {
  ...
  deleteProduct(data, bucket, token) {
    const s3 = new S3({
      accessKeyId: this.configService.get('S3_ACCESS_KEY_ID'),
      secretAccessKey: this.configService.get('S3_SECRET_ACCESS_KEY'),
      endpoint: this.configService.get('S3_ENDPOINT'),
  ...
  delete(data, bucket, token) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // check user owned this shop
        try {
          const query = gql`
            query checkShop($id: Int!) {
              stores_by_pk(
                id: $id
              ) {
                id
          ...
          const r = await graphQLClient.request(query, variables);
          
          if (r.stores_by_pk.id === data.id) {
            try { 
              await s3.deleteObject({
                Bucket: `${this.configService.get('NODE_ENV')}/${bucket}`,
                Key: data.path.replace(
